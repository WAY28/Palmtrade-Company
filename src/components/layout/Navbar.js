  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        <Link href={`/${locale}`} prefetch={true} className="flex items-center gap-2 group">
          ...
          {links.map((link) => (
            <Link key={link.href} href={link.href} prefetch={true}
              className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                ...
                : scrolled ? 'text-gray-700 hover:text-brand-green'
            ...
            {links.map((link) => (
              <Link key={link.href} href={link.href} prefetch={true}
                ...
                className={`px-4 py-3 rounded-lg text-sm font-medium ${
                  isActive(link.href) ? 'bg-brand-light text-brand-dark font-semibold' : 'text-gray-700 hover:bg-gray-50'
                }`}>
                {link.label}
              </Link>
            ))}
